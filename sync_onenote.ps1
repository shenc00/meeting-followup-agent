...
# --- Attempt 1: Graph API download via LoopFetcher (run 'meeting-agent loop-auth' once to cache token) ---
Write-Host "  [a] Searching for Loop notes via Microsoft Graph API..." -ForegroundColor Gray
Push-Location $SCRIPT_DIR
& $VENV_PYTHON -m meeting_agent.cli from-loop --title $meetingTitle
$exitCode = $LASTEXITCODE
Pop-Location
Write-Host ("  Graph API exit code: " + $exitCode) -ForegroundColor Gray
